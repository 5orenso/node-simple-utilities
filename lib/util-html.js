...
renderer.image = function image($href, title, text) {
    ...
    if (result) {
        ...
        if (allClasses[0] === 'cardleft') {
            return `
                <div class="card float-left col-lg-4 col-md-6 col-sm-12 p-0 mr-3 mb-3 mt-3 ${allClasses.join(' ')}">
                    <img class="card-img-top" src="${src}" alt="${title || text}">
                    ${(text || title) ? `<div class="card-body">
        ...
        if (allClasses[0] === 'card') {
            return `
                <div class="card float-right col-lg-4 col-md-6 col-sm-12 p-0 ml-3 mb-3 mt-3 ${allClasses.join(' ')}">
                    <img class="card-img-top" src="${src}" alt="${title || text}">
                    ${(text || title) ? `<div class="card-body">
                        <h5 class="card-title">${text || ''}</h5>
                        <p class="card-text">${title || ''}</p>
        ...
        if (allClasses[0] === 'plainLeft') {
            return `
                <div class='d-flex justify-content-start ${allClasses.join(' ')}'>
                    <img src="${src}" alt="${title || text}" title="${title || text}" class="img-fluid">
                </div>
            `;
        }
        if (allClasses[0] === 'plainRight') {
            return `
                <div class='d-flex justify-content-end ${allClasses.join(' ')}'>
                    <img src="${src}" alt="${title || text}" title="${title || text}" class="img-fluid">
                </div>
            `;
        }
...
class HtmlUtilities {
    ...
    static replaceMarked(input, noPTag) {
        if (typeof input === 'string') {
            ...
            if (noPTag) {
                parsedMarkdown = parsedMarkdown.replace(/^<p>(.+)<\/p>/si, '$1');
            }
