...
class WebUtilities {
    ...
    static renderApi(req, res, opt = {}) {
        const { hrstart, runId } = tc.parseObject(req, '__runTimeData');
        const { routeName, routePath, appPath: applicationPath } = tc.parseObject(req, '__initOpt');
        res.set('Cache-Control', 'no-cache');
        res.set('Vary', 'Origin');
        res.status(opt.response.status).json({
    ...
    static restrictAPI(req, res, next) {
        ...
        return WebUtilities.renderApi(req, res, 403, {
            status: 403,
